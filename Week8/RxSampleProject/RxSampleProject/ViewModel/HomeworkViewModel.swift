...
final class HomeworkViewModel: ViewModelProtocol {
    let disposeBag = DisposeBag()
    
    struct Input {
        let searchButtonTap: ControlEvent<Void>
        let searchText: ControlProperty<String>
        let cellButtonTap: ControlEvent<IndexPath>
    }
    
    struct Output {
        let personList: /*BehaviorSubject*/BehaviorRelay<[Person]>
    }
...
//        let personList = BehaviorSubject(value: sampleUsers)
...
//                        
//                        var all = try! personList.value()
//                        all.insert(Person(name: text, email: "", profileImage: "https://randomuser.me/api/portraits/thumb/women/25.jpg"), at: 0)
//                        personList.on(.next(all))
//                    }
//                    .disposed(by: owner.disposeBag)
//                
//            }
//            .disposed(by: disposeBag)
        // 바인드가 중첩되면 적용해볼 오퍼레이터 없는지 생각해보기
        input.searchButtonTap
            .withLatestFrom(input.searchText)
            .bind(with: self) { owner, text in
                var all = personList.value
                all.insert(Person(name: text, email: "", profileImage: "https://randomuser.me/api/portraits/thumb/women/25.jpg"), at: 0)
                personList.accept(all)
            }
            .disposed(by: disposeBag)
        
        return Output(personList: personList)
    }
}
